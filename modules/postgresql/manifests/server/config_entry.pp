...
define postgresql::server::config_entry (
  ...
  #Allow users to manage postgresql.conf even if they are not managing the whole PostgreSQL instance
  if !defined( 'postgresql::server' ) {
    $manage_postgresql_conf = true
  }
  else {
    $manage_pg_hba_conf = $postgresql::server::manage_postgresql_conf
  }

  if $manage_postgresql_conf == false {
      fail('postgresql::server::manage_postgresql_conf has been disabled, so this resource is now unused and redundant, either enable that option or remove this resource from your manifests')
  }
  else {
